...
function server.setPlayerInventory(player, data)
	...
	if inv then
		...
		TriggerClientEvent('inventory:setPlayerInventory', player.source, Inventory.Drops, inventory, totalWeight, inv.player)
	end
end
exports('setPlayerInventory', server.setPlayerInventory)
AddEventHandler('inventory:setPlayerInventory', server.setPlayerInventory)
...
---@param invType string
---@param data string|number|table
lib.callback.register('inventory:openInventory', function(source, invType, data)
	return openInventory(source, invType, data)
end)

---@param netId number
lib.callback.register('inventory:isVehicleATrailer', function(source, netId)
...
exports('forceOpenInventory', function(playerId, invType, data)
	...
	if left and right then
		TriggerClientEvent('inventory:forceOpenInventory', playerId, left, right)
		return right.id
	end
end)
...
lib.callback.register('inventory:buyLicense', function(source, id)
...
lib.callback.register('inventory:getItemCount', function(source, item, metadata, target)
	local inventory = target and Inventory(target) or Inventory(source)
	return (inventory and Inventory.GetItem(inventory, item, metadata, true)) or 0
end)

lib.callback.register('inventory:getInventory', function(source, id)
...
RegisterNetEvent('inventory:usedItemInternal', function(slot)
    local inventory = Inventory(source)
    ...
    TriggerEvent('inventory:usedItem', inventory.id, item.name, item.slot, next(item.metadata) and item.metadata)
...
lib.callback.register('inventory:useItem', function(source, itemName, slot, metadata)
	...
	if inventory.player then
		...
		if item and data and data.count > 0 and data.name == item.name then
            ...
            ---@type boolean
			local success = lib.callback.await('inventory:usingItem', source, data)
