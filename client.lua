...
RegisterNetEvent('inventory:disarm', function(noAnim)
...
function client.openInventory(inv, data)
		...
		if IsNuiFocused() then
			...
			if inv ~= 'drop' and inv ~= 'container' then
				if (data?.id or data) == currentInventory?.id then
					-- Triggering exports.inventory:openInventory('stash', 'mystash') twice in rapid succession is weird behaviour
	...
	if canOpenInventory() then
		...
		if inv == 'shop' and invOpen == false then
			...
			left, right = lib.callback.await('inventory:openShop', 200, data)
		elseif inv == 'crafting' then
			if cache.vehicle then
				return lib.notify({ id = 'cannot_perform', type = 'error', description = locale('cannot_perform') })
			end

			left = lib.callback.await('inventory:openCraftingBench', 200, data.id, data.index)
		...
		elseif invOpen ~= nil then
			...
			left, right = lib.callback.await('inventory:openInventory', false, inv, data)
		end
...
RegisterNetEvent('inventory:openInventory', client.openInventory)
exports('openInventory', client.openInventory)

RegisterNetEvent('inventory:forceOpenInventory', function(left, right)
...
lib.callback.register('inventory:usingItem', function(data)
...
local function useItem(data, cb)
    ...
    usingItem = true
    ---@type boolean?
    result = lib.callback.await('inventory:useItem', 200, data.name, data.slot, slotData.metadata)
    ...
    if result then
        TriggerEvent('inventory:usedItem', slotData.name, slotData.slot, next(slotData.metadata) and slotData.metadata)
    end
...
AddEventHandler('inventory:usedItem', function(name, slot, metadata)
    TriggerServerEvent('inventory:usedItemInternal', slot)
end)

AddEventHandler('inventory:item', useItem)
		...
		elseif currentWeapon then
			if data.ammo then
				...
				useItem(data, function(resp)
					...
					lib.callback.await('inventory:updateWeapon', false, 'load', newAmmo, false, currentWeapon.metadata.specialAmmo)
				end)
					...
					if DoesWeaponTakeWeaponComponent(currentWeapon.hash, component) then
						if HasPedGotWeaponComponent(playerPed, currentWeapon.hash, component) then
							...
							useItem(data, function(data)
								if data then
									local success = lib.callback.await('inventory:updateWeapon', false, 'component', tostring(data.slot), currentWeapon.slot)

									if success then
										GiveWeaponComponentToPed(playerPed, currentWeapon.hash, component)
										TriggerEvent('inventory:updateWeaponComponent', 'added', component, data.name)
									end
								end
							end)
						end
...
RegisterNetEvent('inventory:closeInventory', client.closeInventory)
...
local function updateInventory(data, weight)
		...
		if not v.inventory or v.inventory == cache.serverId then
			...
			if currentWeapon?.slot == item?.slot then
                if item.metadata then
				    ...
				    TriggerEvent('inventory:currentWeapon', currentWeapon)
                else
	...
	for itemName, count in pairs(itemCount) do
        ...
        if item then
            ...
            TriggerEvent('inventory:itemCount', item.name, item.count)
	...
	TriggerEvent('inventory:updateInventory', changes)
end

RegisterNetEvent('inventory:updateSlots', function(items, weights)
	if source ~= '' and next(items) then updateInventory(items, weights) end
end)

RegisterNetEvent('inventory:inventoryReturned', function(data)
	if source == '' then return end
...
RegisterNetEvent('inventory:inventoryConfiscated', function(message)
	if source == '' then return end
...
RegisterNetEvent('inventory:createDrop', function(dropId, data, owner, slot)
...
RegisterNetEvent('inventory:setPlayerInventory', function(currentDrops, inventory, weight, player)
	if source == '' then return end
	...
	local function nearbyLicense(point)
		...
		if point.isClosest and point.currentDistance < 1.2 then
			...
			if IsControlJustReleased(0, 38) then
				lib.callback('inventory:buyLicense', 1000, function(success, message)
					if success ~= nil then
						lib.notify({
							id = message,
	...
	TriggerEvent('inventory:updateInventory', PlayerData.inventory)
		...
		if invOpen then
			...
			if currentWeapon and currentWeapon.timer then
				...
				if not invBusy and currentWeapon.timer ~= 0 and currentWeapon.timer < GetGameTimer() then
					...
					if weaponAmmo then
						TriggerServerEvent('inventory:updateWeapon', 'ammo', weaponAmmo)
					...
					elseif currentWeapon.metadata.durability then
						TriggerServerEvent('inventory:updateWeapon', 'melee', currentWeapon.melee)
				...
				elseif currentWeapon.throwable then
					if not invBusy and IsControlPressed(0, 24) then
						...
						CreateThread(function()
							...
							TriggerServerEvent('inventory:updateWeapon', 'throw', nil, weapon.slot)

							plyState.invBusy = false
							currentWeapon = nil

							RemoveWeaponFromPed(playerPed, weapon.hash)
							TriggerEvent('inventory:currentWeapon')
						end)
					end
...
RegisterNetEvent('inventory:viewInventory', function(left, right)
	if source == '' then return end
...
RegisterNUICallback('removeComponent', function(data, cb)
	...
	if not currentWeapon then
		return TriggerServerEvent('inventory:updateWeapon', 'component', data)
	end
		...
		if HasPedGotWeaponComponent(playerPed, currentWeapon.hash, component) then
			for k, v in pairs(itemSlot.metadata.components) do
				if v == data.component then
					local success = lib.callback.await('inventory:updateWeapon', false, 'component', k)

					if success then
						RemoveWeaponComponentFromPed(playerPed, currentWeapon.hash, component)
						TriggerEvent('inventory:updateWeaponComponent', 'removed', component, data.component)
					end
...
RegisterNUICallback('removeAmmo', function(slot, cb)
	...
	local success = lib.callback.await('inventory:removeAmmoFromWeapon', false, slot)
...
local function giveItemToTarget(serverId, slotId, count)
    ...
    TriggerServerEvent('inventory:giveItem', slotId, serverId, count or 0)
end
...
RegisterNUICallback('giveItem', function(data, cb)
	...
	if client.giveplayerlist then
		...
		return lib.showMenu('inventory:givePlayerList')
	end
...
lib.callback.register('inventory:startCrafting', function(id, recipe)
...
RegisterNUICallback('swapItems', function(data, cb)
	...
	local success, response, weaponSlot = lib.callback.await('inventory:swapItems', false, data)
    swapActive = false
...
RegisterNUICallback('buyItem', function(data, cb)
	...
	local response, data, message = lib.callback.await('inventory:buyItem', 100, data)
...
RegisterNUICallback('craftItem', function(data, cb)
	...
	for i = 1, data.count do
		local success, response = lib.callback.await('inventory:craftItem', 200, id, index, data.fromSlot, data.toSlot)
...
lib.callback.register('inventory:getVehicleData', function(netid)
