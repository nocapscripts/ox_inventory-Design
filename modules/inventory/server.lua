...
function OxInventory:openInventory(inv)
	...
	TriggerEvent('inventory:openedInventory', self.id, inv.id)
end
...
function OxInventory:closeInventory(noEvent)
	...
	if not noEvent then
		TriggerClientEvent('inventory:closeInventory', self.id, true)
	end

	TriggerEvent('inventory:closedInventory', self.id, inv.id)
end
...
function OxInventory:syncSlotsWithPlayer(slots, weight)
	TriggerClientEvent('inventory:updateSlots', self.id, slots, weight)
end
...
function OxInventory:syncSlotsWithClients(slots, syncOwner)
	for playerId in pairs(self.openedBy) do
		if self.id ~= playerId then
            ...
            if target then
			    TriggerClientEvent('inventory:updateSlots', playerId, slots, target.weight)
            end
		end
	end

	if syncOwner and self.player then
		TriggerClientEvent('inventory:updateSlots', self.id, slots, self.weight)
	end
end
...
local function loadInventoryData(data, player)
	...
	if data.type == 'trunk' or data.type == 'glovebox' then
		...
		if not inventory then
			...
			local model, class = lib.callback.await('inventory:getVehicleData', source, data.netid)
...
function Inventory.CloseAll(inv, ignoreId)
	if not inv then
		...
		return TriggerClientEvent('inventory:closeInventory', -1, true)
	end
...
function Inventory.SetSlot(inv, item, count, metadata, slot)
	...
	if currentSlot and newCount < 1 then
		TriggerClientEvent('inventory:itemNotify', inv.id, { currentSlot, 'ui_removed', currentSlot.count })
		currentSlot = nil
	else
		...
		TriggerClientEvent('inventory:itemNotify', inv.id, { currentSlot, count < 0 and 'ui_removed' or 'ui_added', math.abs(count) })
	end
...
CreateThread(function()
    ...
    TriggerEvent('inventory:loadInventory', Inventory)
end)
...
--- To create a stash, please use `exports.inventory:RegisterStash` instead.
...
function Inventory.Remove(inv)
	...
	if inv then
		if inv.type == 'drop' then
			TriggerClientEvent('inventory:removeDrop', -1, inv.id)
...
function Inventory.SetSlotCount(inv, slots)
	...
	if inv.player then
        TriggerClientEvent('inventory:refreshSlotCount', inv.id, {inventoryId = inv.id, slots = inv.slots})
    end

    for playerId in pairs(inv.openedBy) do
        if playerId ~= inv.id then
            TriggerClientEvent('inventory:refreshSlotCount', playerId, {inventoryId = inv.id, slots = inv.slots})
        end
	end
end
...
function Inventory.SetMaxWeight(inv, maxWeight)
    ...
    if inv.player then
        TriggerClientEvent('inventory:refreshMaxWeight', inv.id, {inventoryId = inv.id, maxWeight = inv.maxWeight})
    end

    for playerId in pairs(inv.openedBy) do
        if playerId ~= inv.id then
            TriggerClientEvent('inventory:refreshMaxWeight', playerId, {inventoryId = inv.id, maxWeight = inv.maxWeight})
        end
	end
end
		...
		elseif itemSlots and totalCount > 0 then
			for k, v in pairs(itemSlots) do
				if removed < total then
					if v == count then
						TriggerClientEvent('inventory:itemNotify', inv.id, { inv.items[k], 'ui_removed', v })
...
---@param metadata { [string]: any }
---@param slot number
RegisterServerEvent('inventory:removeItem', function(name, count, metadata, slot)
...
local function CustomDrop(prefix, items, coords, slots, maxWeight, instance, model)
	...
	TriggerClientEvent('inventory:createDrop', -1, dropId, Inventory.Drops[dropId])

    return dropId
end

AddEventHandler('inventory:customDrop', CustomDrop)
...
local function dropItem(source, playerInventory, fromData, data)
	...
	TriggerClientEvent('inventory:createDrop', -1, dropId, Inventory.Drops[dropId], playerInventory.open and source, slot)
...
---@param source number
---@param data SwapSlotData
lib.callback.register('inventory:swapItems', function(source, data)
		...
		if fromData then
				...
				if not sameInventory then
					if (toWeight <= toInventory.maxWeight and fromWeight <= fromInventory.maxWeight) then
						...
						if fromOtherPlayer then
							TriggerClientEvent('inventory:itemNotify', fromInventory.id, { fromData, 'ui_removed', fromData.count })
							TriggerClientEvent('inventory:itemNotify', fromInventory.id, { toData, 'ui_added', toData.count })
						elseif toOtherPlayer then
							TriggerClientEvent('inventory:itemNotify', toInventory.id, { fromData, 'ui_added', fromData.count })
							TriggerClientEvent('inventory:itemNotify', toInventory.id, { toData, 'ui_removed', toData.count })
						end
			...
			elseif toData and toData.name == fromData.name and table.matches(toData.metadata, fromData.metadata) then
				...
				if fromInventory.type == 'container' or sameInventory or totalWeight <= toInventory.maxWeight then
					...
					if not sameInventory then
						...
						if fromOtherPlayer then
							TriggerClientEvent('inventory:itemNotify', fromInventory.id, { fromData, 'ui_removed', data.count })
						elseif toOtherPlayer then
							TriggerClientEvent('inventory:itemNotify', toInventory.id, { toData, 'ui_added', data.count })
						end
				...
				if fromInventory.type == 'container' or sameInventory or (toInventory.weight + toData.weight <= toInventory.maxWeight) then
					...
					if not sameInventory then
						...
						if fromOtherPlayer then
							TriggerClientEvent('inventory:itemNotify', fromInventory.id, { fromData, 'ui_removed', data.count })
						elseif toOtherPlayer then
							TriggerClientEvent('inventory:itemNotify', toInventory.id, { fromData, 'ui_added', data.count })
						end
			...
			if toInventory.weapon == data.toSlot then
				if not sameInventory then
					...
					TriggerClientEvent('inventory:disarm', toInventory.id)
				else
			...
			if fromInventory.weapon == data.fromSlot then
				if not sameInventory then
					...
					TriggerClientEvent('inventory:disarm', fromInventory.id)
...
function Inventory.Confiscate(source)
	...
	if inv?.player then
		...
		TriggerClientEvent('inventory:inventoryConfiscated', inv.id)
...
function Inventory.Return(source)
    ...
    TriggerClientEvent('inventory:inventoryReturned', source, { inventory, totalWeight })
...
RegisterServerEvent('inventory:closeInventory', function()
...
RegisterServerEvent('inventory:giveItem', function(slot, target, count)
...
lib.callback.register('inventory:updateWeapon', updateWeapon)

RegisterNetEvent('inventory:updateWeapon', function(action, value, slot, specialAmmo)
	updateWeapon(source, action, value, slot, specialAmmo)
end)

lib.callback.register('inventory:removeAmmoFromWeapon', function(source, slot)
...
function Inventory.InspectInventory(playerId, invId)
	...
	if playerInventory and inventory then
		...
		TriggerClientEvent('inventory:viewInventory', playerId, playerInventory, inventory)
	end
end
