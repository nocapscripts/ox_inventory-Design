...
lib.callback.register('inventory:openCraftingBench', function(source, id, index)
...
lib.callback.register('inventory:craftItem', function(source, id, index, recipeId, toSlot)
	local left, bench = Inventory(source), CraftingBenches[id]
	...
	if bench then
		...
		if recipe then
			...
			local success = lib.callback.await('inventory:startCrafting', source, id, recipeId)
